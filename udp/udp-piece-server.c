...
void savePic() {
    FILE* fp;
    fp = fopen("/data/test/3516.png", "wb+"); // localfile文件名
    fwrite(udp_piece->recv_buf, udp_piece->recv_len, 1, fp);
    fclose(fp);
}
...
void echo_server(int sock) {
    ...
    // 初始化
    udp_piece = udp_piece_init(1024 * 1024 * 3);
    while (1) {
        peerlen = sizeof(peeraddr);
        memset(recvbuf, 0, sizeof(recvbuf));
        n = (int) recvfrom(sock, recvbuf, sizeof(recvbuf), 0,
